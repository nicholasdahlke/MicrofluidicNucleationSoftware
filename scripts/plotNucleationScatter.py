...
if __name__ == "__main__":
    ...
    ax.plot(fit_x, fit_lin[0]*fit_x*kB + fit_lin[1]*kB, linewidth=1.5, color="black", label="Fit", zorder=1)
    marker_size = 60
    param[:, 1] = param[:, 1]*kB
    ax.scatter(param[:num_warm, 0], param[:num_warm, 1], c=color_orange,label=r"$T_h = 70^\circ\text{C}$", zorder=2, s=marker_size)
    ax.scatter(param[num_warm:, 0], param[num_warm:, 1], c=color_blue, label=r"$T_h = 30^\circ\text{C}$", zorder=2, s=marker_size)

    covariance_data = np.cov(np.transpose(param))
    eigenvalues, eigenvectors = np.linalg.eig(covariance_data)
